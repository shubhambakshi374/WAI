...
class ProviderPicker(ModalScreen[str | None]):
    ...
    def _options(self) -> list[Option]:
        from altus.config.secrets import USES_CREDENTIAL_CHAIN, credential_status
        from altus.providers.local import LocalProvider, configured_endpoints

        endpoints = dict(configured_endpoints(self.altus.config))
        current = self.altus.session.provider
        options: list[Option] = []
