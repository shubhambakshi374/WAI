...
from altus.core.errors import CredentialsError

log = logging.getLogger(__name__)

KEYRING_SERVICE = "altus"

#: The service name keys were stored under before the rename. Read but never
#: written: an existing keychain entry keeps working, and anything set from
#: here on lands under the current name.
LEGACY_KEYRING_SERVICE = "wai"
...
def _altus_env_var(provider: str) -> str:
    return f"ALTUS_{provider.upper()}_API_KEY"


def _env_vars(provider: str) -> tuple[str, ...]:
    """Ours first, then the pre-rename spelling, then the vendor's own.

    The old spelling is still read for the same reason the old config
    directory is: it is in people's shell profiles, and a rename is not a
    reason to break those.
    """
    return (
        _altus_env_var(provider),
        f"WAI_{provider.upper()}_API_KEY",
        *NATIVE_ENV_VARS.get(provider, ()),
    )


def get_api_key(provider: str) -> str | None:
    """Resolve a key: ``ALTUS_<PROVIDER>_API_KEY`` -> native env var -> keyring."""
...
def _env_key(provider: str) -> str | None:
    ...
    return os.environ.get(_altus_env_var(provider)) or os.environ.get(
        f"WAI_{provider.upper()}_API_KEY"
    )
...
def require_api_key(provider: str) -> str:
    ...
    if not key:
        raise CredentialsError(
            f"no API key for {provider!r}. Set {_altus_env_var(provider)} "
            f"or run: altus config set-key {provider}",
            provider=provider,
        )
    return key
...
def _keyring_get(provider: str) -> str | None:
    try:
        ...
        return keyring.get_password(KEYRING_SERVICE, provider) or keyring.get_password(
...
def credential_status(provider: str) -> CredentialStatus:
    ...
    return CredentialStatus(provider, False, "none", "run: altus config set-key " + provider)
