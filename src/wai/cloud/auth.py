...
def _azure_status() -> CloudStatus:
    ...
    wai_cache = _azure_cache_path()
    if wai_cache.exists():
        return CloudStatus("azure", True, True, source="wai-device-code", detail=str(wai_cache))
...
def _azure_cache_path() -> Path:
    from wai.config.loader import data_dir
...
async def _login_azure() -> tuple[bool, str]:
    ...
    try:
        ...
        return False, "azure support is not installed: uv tool install 'wai[azure]'"
    ...
    def _acquire() -> str:
        credential = DeviceCodeCredential(
            ...
            cache_persistence_options=TokenCachePersistenceOptions(name="wai"),
        )
